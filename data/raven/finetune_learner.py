...
from espnet.asr.asr_utils import add_results_to_json, add_results_to_json_all, torch_load
...
class Learner(LightningModule):
    ...
    def get_nbest_hyps(self, x):
        """
        x: (B, T, H, W) tensor, channel dim squeezed
        """
        assert self.cfg.data.modality == "video"
        feat, _ = self.model.encoder(x.unsqueeze(0), None)
        ...
        transcriptions, scores = add_results_to_json_all(nbest_hyps, self.token_list)
